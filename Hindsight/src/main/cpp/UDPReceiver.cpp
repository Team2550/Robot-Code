...
double UDPReceiver::getUDPDataAge() { return double(UDPAgeTimer.Get()); }
...
================================================*/
bool UDPReceiver::getUDPDataIsReal() { return isRealData; }
...
bool UDPReceiver::checkUDP() {
	...
	try {
		bytesRecievedCount =
			recvfrom(ourSocket, buffer, BUFSIZE, MSG_DONTWAIT,
	...
	if (bytesRecievedCount > 0) {
		...
		std::vector<std::string> dataPointsStrings =
			Utility::SplitString(bufferString, ',');

		std::vector<std::vector<float>> dataPoints;

		std::vector<float> currentDataPoint;

		// Convert to float and sort data points by size from smallest to
		// greatest distance
		for (unsigned int i = 0; i < dataPointsStrings.size(); i++) {
			currentDataPoint = Utility::StrVectorToFloatVector(
				Utility::SplitString(dataPointsStrings[i], ' '));

			// If data point is the right length, add it to the right position
			// in the dataPoints vector (sorted by percent match)
			if (currentDataPoint.size() == UDP::DataCount)
				for (unsigned int j = 0; j <= dataPoints.size(); j++)
					if (j >= dataPoints.size() ||
						currentDataPoint[UDP::Index::PercentMatch] >
							dataPoints[j][UDP::Index::PercentMatch]) {
						dataPoints.insert(dataPoints.begin() + j,
										  currentDataPoint);
						j = dataPoints.size() + 1;
					}
		}
		...
		if (dataPoints.size() > 1) {
			// If one target is a much worse match than the other, ignore the
			// data
			if (dataPoints[1][UDP::Index::PercentMatch] /
					dataPoints[0][UDP::Index::PercentMatch] >
				0.75) {
				// Average two best matches
				for (unsigned int i = 0; i < UDP::DataCount; i++)
					newestUDPData[i] =
						(dataPoints[0][i] + dataPoints[1][i]) / 2;
				UDPAgeTimer.Reset();
				isRealData = true;
